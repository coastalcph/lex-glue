...
class HierarchicalConvolutionalBert(nn.Module):
    def __init__(self, encoder, max_segments=[64, 32, 16], max_segment_length=[128, 256, 512],num_labels=None):
        ...
        self.seg_pos_embeddings = nn.Embedding(max_segments[0] + 1, encoder.config.hidden_size,
                                               padding_idx=0,
                                               _weight=sinusoidal_init(max_segments[0] + 1, encoder.config.hidden_size))
    ...
    def forward(self, input_ids_1=None, attention_mask_1=None, token_type_ids_1=None, 
        ...
        for i in range(1,4):
            ...
            seg_positions=seg_positions*(2**(i-1))-(2**(i-1)-1)
